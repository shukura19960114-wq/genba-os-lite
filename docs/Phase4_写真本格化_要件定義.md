...
- **追加（任意）**：`photoCountProvider`（= `photosProvider` の件数を派生）で枚数表示。
- `PhotoUploadController` に **`addPhotos(...)` を追加**（ライブラリ複数選択→ループupload→`photosProvider(siteId)` を invalidate）。
  - 戻り値で「成功枚数 / 失敗枚数 / キャンセル」を返し、画面で SnackBar 表示。
  - 既存 `addPhoto`（1枚・カメラ用）は維持。
- ビューアの現在indexは画面ローカル state（`PageController`）で保持（provider不要）。
